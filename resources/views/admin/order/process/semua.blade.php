    ...
    <tbody>
        @foreach($order as $index => $od)
            <tr>
                ...
                <td class="hide-on-small text-center">
                    ...
                    <a href="{{ route('orderProcess.actionDiantar', $od->id) }}">
                        <button class="btn btn-success mt-2"><i class="ri-truck-fill"></i></button>
                    </a>
                    @elseif ($od->delivery_option == 'dine-in' && $od->status_pembayaran == 'pending')
                    <a href="{{ route('orderProcess.actionDibayar', $od->id) }}">
                        <button class="btn btn-success mt-2"><i class="ri-money-dollar-box-fill"></i></button>
                    </a>
                    @elseif ($od->delivery_option == 'delivery' && $od->status_pembayaran == 'capture')
                    <a href="{{ route('orderProcess.actionSelesai', $od->id) }}">
                        <button type="submit" class="btn btn-success mt-2"><i class="ri-check-double-line"></i></button>
                    </a>
            ...
            <div class="tableSmall appear-on-small">
                ...
                <p class="appear-on-small justify-content-between"><strong>Status Pesanan:</strong>
                    @if ($od->status == 'Menunggu Diproses')
                        <span class="badge text-bg-warning py-2">{{ $od->status }}</span>
                    @elseif ($od->status == 'Sedang Diproses')
                        <span class="badge text-bg-info py-2">{{ $od->status }}</span>
                ...
                <p class="appear-on-small text-center d-flex justify-content-center align-items-center gap-2">
                    ...
                    @if ($od->delivery_option == 'delivery' && $od->status_pembayaran == 'pending')
                    <a href="{{ route('orderProcess.actionDiantar', $od->id) }}">
                        <button class="smlInBtn btn btn-success d-flex align-items-center"><i class="ri-truck-fill"></i>Sudah Diterima</button>
                    </a>
                    @elseif ($od->delivery_option == 'dine-in' && $od->status_pembayaran == 'pending')
                    <a href="{{ route('orderProcess.actionDibayar', $od->id) }}">
                        <button class="smlInBtn btn btn-success d-flex align-items-center"><i class="ri-money-dollar-box-fill"></i>Sudah Dibayar</button>
                    </a>
                    @elseif ($od->delivery_option == 'delivery' && $od->status_pembayaran == 'capture')
                    <a href="{{ route('orderProcess.actionSelesai', $od->id) }}">
                        <button type="submit" class="smlInBtn btn btn-success d-flex align-items-center"><i class="ri-check-double-line"></i>Selesai</button>
                    </a>
                    @elseif ($od->delivery_option == 'dine-in' && $od->status_pembayaran == 'capture')
                    <a href="{{ route('orderProcess.actionSelesai', $od->id) }}">
                        <button type="submit" class="smlInBtn btn btn-success d-flex align-items-center"><i class="ri-check-double-line"></i>Selesai</button>
